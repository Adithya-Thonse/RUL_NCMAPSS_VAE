...
def get_callbacks(model, data, target, filepath='./checkpoints/checkpoint'):
    model_callbacks = [
        EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=30),
        ModelCheckpoint(filepath=filepath, monitor='val_loss', mode='min', verbose=1,
...
def score(y_true, y_hat):
    ...
    print("NASA score: ", res/len(y_hat))
