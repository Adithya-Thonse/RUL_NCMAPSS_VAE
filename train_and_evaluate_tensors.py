...
def arg_parser():
    ...
    parser.add_argument('-load_weights_from', help='Instead of training, use checkpoint directory to load weights', type=mdcl_utils.path_arg,
...
def main():
    ...
    if args.load_weights_from:
        ...
        for filename_suffix in filename_suffix_list:
            x_train = tf.constant(numpy_loader(opj(data_dir, 'x_train_{}.npy'.format(filename_suffix))))
            x_val = tf.constant(numpy_loader(opj(data_dir, 'x_val_{}.npy'.format(filename_suffix))))
            y_train = tf.constant(numpy_loader(opj(data_dir, 'y_train_{}.npy'.format(filename_suffix))))
            y_val = tf.constant(numpy_loader(opj(data_dir, 'y_val_{}.npy'.format(filename_suffix))))
    ...
    for filename_suffix in filename_suffix_list:
        ...
        x_test = tf.constant(numpy_loader(opj(data_dir, 'x_test_{}.npy'.format(filename_suffix))))
        y_test = tf.constant(numpy_loader(opj(data_dir, 'y_test_{}.npy'.format(filename_suffix))))
    ...
    for filename_suffix in filename_suffix_list:
        ...
        del y_test
        gc.collect()
    utils.evaluate(tf.constant(y_dev_total), tf.constant(y_hat_train), 'train')
    del y_dev_total
    del y_hat_train
    gc.collect()
    utils.evaluate(tf.constant(y_test_total), tf.constant(y_hat_test), 'test')
    utils.score(tf.constant(y_test_total), tf.constant(y_hat_test))
