...
def main():
    ...
    if args.load_weights_from:
        ...
        for filename_suffix in filename_suffix_list:
            x_train = numpy_loader(opj(data_dir, 'x_train_{}.npy'.format(filename_suffix)))
    ...
    for filename_suffix in filename_suffix_list:
        ...
        for x_sub_array in np.array_split(np.concatenate((x_train, x_val)), 1000):
            y_pred_train = RVE(x_sub_array)
            # logger.info(z.shape)
            train_mu = np.concatenate((train_mu, y_pred_train))
        ...
        x_test = numpy_loader(opj(data_dir, 'x_test_{}.npy'.format(filename_suffix)))
        # y_test = numpy_loader(opj(data_dir, 'y_test_{}.npy'.format(filename_suffix)))
        # y_test = y_test.reshape(len(y_test), 1)
        for x_sub_array in np.array_split(x_test, 1000):
            y_pred_test = RVE(x_sub_array)
            test_mu = np.concatenate((test_mu, y_pred_test))
        del x_test
        # del y_test
        gc.collect()
    y_hat_train = train_mu#RVE.regressor.predict(train_mu)
    y_hat_test = test_mu# RVE.regressor.predict(test_mu)
    # nasa_score = 0
